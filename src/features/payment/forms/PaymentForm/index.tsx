...
const PaymentForm = () => {
  ...
  const setFormData = useCallback(
    (inputName: string, value: boolean | boolean) => {
  ...
  const formSubmitHandler = (event: React.FormEvent<HTMLFormElement>) => {
    ...
    if (
      (emptyFields?.[0]?.[0] === "lastName" && emptyFields.length === 1) ||
  ...
  return (
    ...
    >
      <FormHeading />
      <Form method="POST" autoComplete="off" onSubmit={formSubmitHandler}>
        <FirstName />
        <LastName />
        <CardNumber />
        <ExpirationDate />
  ...
  return (
    <AgreementContainer>
      ...
      <p>
        ...
        <Link to="">Terms of Use</Link>,<Link to="">Privacy Statement</Link>,
        and that you are over 18. Netflix will automatically continue your
        membership and charge the membership fee
        {` (currently ${planInfo.planPrice}/month) `}to your payment method
