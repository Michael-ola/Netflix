...
const Form = () => {
  ...
  const onSubmit: SubmitHandler<IFormInput> = (data) => {
    ...
    if (contact && password) {
      dispatch(
        auth(contact, password, false, undefined, () =>
  ...
  return (
    ...
    >
      ...
      <SubmitButton spinner={loadingState} onClick={buttonClickedHandler}>
        Sign In
      </SubmitButton>
...
const ContactInput = ({
  ...
  const userContactInput = useInput({
    errors,
    getValues,
    register,
    submitButtonState: submitButtonClicked,
...
const MetaText = () => {
  return (
    <MetaTextContainer>
      <h3>
        New to Netflix? <SignUpLink to="/">Sign UP now.</SignUpLink>
      </h3>
      <h5>
